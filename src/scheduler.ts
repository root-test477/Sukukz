...
interface ScheduledTask {
  ...
  executed: boolean;
  createdBy: number;
  botId: string; // Add botId to track which bot the task belongs to
}
 ...
 */
async function notifyAdmin(adminId: number, message: string, botId: string): Promise<void> {
  try {
    await safeSendMessage(adminId, message, undefined, botId);
  } catch (error) {
 ...
 */
export async function handleScheduleCommand(msg: TelegramBot.Message, botId: string): Promise<void> {
  ...
  if (commandArgs.length === 0) {
    await safeSendMessage(chatId, "⚠️ Please provide a message to send", undefined, botId);
    return;
  }
  ...
  await safeSendMessage(chatId, confirmationMsg, { parse_mode: 'Markdown' }, botId);
}
