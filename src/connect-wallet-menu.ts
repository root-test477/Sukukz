...
// Default botId - will be overridden when the functions are called
let currentBotId = '1';
...
async function onChooseWalletClick(query: CallbackQuery, _: string, botId?: string): Promise<void> {
    // Use provided botId or default
    const activeBotId = botId || currentBotId;
    // Store the botId for subsequent operations
    ...
    try {
        ...
        const bot = botFactory.getBot(activeBotId);
        if (!bot) {
            console.error(`Bot with ID ${activeBotId} not found`);
            return;
        }
...
async function onOpenUniversalQRClick(query: CallbackQuery, _: string, botId?: string): Promise<void> {
    // Use provided botId or default
    const activeBotId = botId || currentBotId;
    // Store the botId for subsequent operations
    currentBotId = activeBotId;
    ...
    try {
        const wallets = await getWallets();
        const connector = getConnector(chatId, activeBotId);
...
async function onWalletClick(query: CallbackQuery, data: string, botId?: string): Promise<void> {
    // Use provided botId or default
    const activeBotId = botId || currentBotId;
    // Store the botId for subsequent operations
    currentBotId = activeBotId;
...
async function editQR(message: TelegramBot.Message, link: string, botId?: string): Promise<void> {
    // Use provided botId or default
    const activeBotId = botId || currentBotId;
