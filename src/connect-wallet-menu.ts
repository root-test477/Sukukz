...
async function onWalletClick(query: CallbackQuery, data: string, botId: string): Promise<void> {
    ...
    try {
        ...
        if (isTelegramUrl(selectedWallet.universalLink)) {
            ...
            const botLink = botManager.getBotConfig(botId)?.link || process.env.TELEGRAM_BOT_LINK || 'https://t.me/your_bot';
            buttonLink = addTGReturnStrategy(buttonLink, botLink);
            qrLink = addTGReturnStrategy(qrLink, 'none');
        }

        try {
            await editQR(query.message, qrLink, botId);
        } catch (qrError) {
...
async function editQR(message: TelegramBot.Message, link: string, botId?: string): Promise<void> {
    ...
    const bot = botId ? botManager.getBot(botId) : botManager.getBot('main');
    if (!bot) {
