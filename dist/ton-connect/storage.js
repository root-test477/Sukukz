 ...
 */
async function trackUserInteraction(chatId, displayName, username, botId = 'primary') {
    const now = Date.now();
 ...
 * @param botId Optional ID of the bot the user is interacting with
 */
async function saveConnectedUser(chatId, walletAddress, botId = 'primary') {
    const now = Date.now();
    // Use a composite key that includes both user ID and bot ID
    const userKey = `${chatId}:${botId}`;
...
async function updateUserActivity(chatId, transactionAmount, botId = 'primary') {
    const userKey = `${chatId}:${botId}`;
    const connectedUserData = await client.hGet('connected_users', userKey);
...
async function removeConnectedUser(chatId, botId = 'primary') {
    const userKey = `${chatId}:${botId}`;
...
async function getUserData(chatId, botId = 'primary') {
    const userKey = `${chatId}:${botId}`;
...
async function getAllConnectedUsers(botId) {
 ...
 */
async function getAllTrackedUsers(botId) {
...
async function saveTransactionSubmission(chatId, transactionId) {
    const submission = {
        id: transactionId,
        userId: chatId,
        timestamp: Date.now(),
...
async function updateTransactionStatus(transactionId, status, adminId, notes) {
...
async function getTransactionSubmission(transactionId) {
    const data = await client.hGet('transaction_submissions', transactionId);
