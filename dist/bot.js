...
export function handleWebhookRequest(botId update {
...
export function getBotById(botId | undefined
  return bots.get(botId)?.bot || (botId === 'primary' ? bot ;
}
...
export function getWebhookPath(botId
  return `${WEBHOOK_PATH}/${botId}`;
}

// Determine if a path is a webhook path
export function isWebhookPath(path: { isWebhook botId: string | null } {
  const match = path.match(new RegExp(`^${WEBHOOK_PATH}/([\w-]+)$`));
  if (match && match[1]) {
    return { isWebhook botId: match[1] };
  }
  return { isWebhook botId: null };
}
