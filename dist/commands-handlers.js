...
// Use composite key (chatId to track connection requests across bots
let newConnectRequestListenersMap = new Map<string, () => void>();

// Helper to get botId from message
function getBotIdFromMessage(msg: TelegramBot.Message)
...
export async function handleSendTXCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const botId = getBotIdFromMessage(msg);
...
export async function handleDisconnectCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const botId = getBotIdFromMessage(msg);
    const botInstance = getBotById(botId) || bot;
 ...
 */
export async function safeRestoreConnection(connector chatId botId 'primary')
    try {
...
export async function handleShowMyWalletCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const botId = getBotIdFromMessage(msg);
    const botInstance = getBotById(botId) || bot;
 ...
 */
export async function handleFundingCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const text = msg.text || '';
 ...
 */
export async function handleSupportCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const text = msg.text || '';
 ...
 */
export async function handlePayNowCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const text = msg.text || '';
    const userIsAdmin = isAdmin(chatId);
    ...
    if (!transactionMatch) {
        ...
        await safeSendMessage(
            ...
            { 
                parse_mode: 'Markdown',
                reply_markup: {
                    inline_keyboard text: '« Back to Menu', callback_data: JSON.stringify({ method: 'back_to_menu', data: '' }) }
                    ]]
                }
            }
        );
    ...
    if (existingSubmission) {
        ...
        switch (existingSubmission.status) {
            case 'pending' 'This transaction ID has already been submitted and is pending review.';
                break;
            case 'approved' 'This transaction ID has already been approved.';
                break;
            case 'rejected' 'This transaction ID was previously rejected. Please submit a new transaction or contact support.';
                break;
        }
 ...
 */
export async function handleApproveCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const text = msg.text || '';
 ...
 */
export async function handleRejectCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const text = msg.text || '';
 ...
 */
export async function handleBackToMenuCallback(query: TelegramBot.CallbackQuery)
 ...
 */
export async function handleWithdrawCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
 ...
 */
export async function handleUsersCommand(msg: TelegramBot.Message)
    const chatId = msg.chat.id;
    const botId = getBotIdFromMessage(msg);
    const botInstance = getBotById(botId) || bot;
    ...
    try {
        ...
        for (const user of allUsers) {
            ...
            let currentWalletInfo | null null;
