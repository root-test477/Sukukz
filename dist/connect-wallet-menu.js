...
async function onChooseWalletClick(query, _) {
    ...
    try {
        ...
        await bot_1.bot.editMessageReplyMarkup({
            inline_keyboard: [
                wallets.map(wallet => ({
                    text: wallet.name,
        ...
        try {
            await (0, error_boundary_1.safeSendMessage)(chatId, 'Sorry, there was an error displaying wallet options. Please try /connect again.');
        }
        catch (sendError) {
...
async function onOpenUniversalQRClick(query, _) {
    ...
    try {
        ...
        const keyboard = await (0, utils_1.buildUniversalKeyboard)(link, wallets);
        await bot_1.bot.editMessageReplyMarkup({
        ...
        try {
            await (0, error_boundary_1.safeSendMessage)(chatId, 'Sorry, there was an error displaying connection options. Please try /connect again.');
        }
        catch (sendError) {
...
async function onWalletClick(query, data) {
    ...
    try {
        const connector = (0, connector_1.getConnector)(chatId);
        const selectedWallet = await (0, wallets_1.getWalletInfo)(data);
        if (!selectedWallet) {
            await (0, error_boundary_1.safeSendMessage)(chatId, 'Selected wallet could not be found. Please try /connect again.');
            return;
        }
        ...
        try {
            await (0, error_boundary_1.safeSendMessage)(chatId, 'Sorry, there was an error setting up wallet connection. Please try /connect again.');
        }
        catch (sendError) {
...
async function editQR(message, link) {
    ...
    try {
        await qrcode_1.default.toFile(`./${fileName}`, link);
        await bot_1.bot.editMessageMedia({
            type: 'photo',
