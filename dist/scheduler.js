 ...
 */
function scheduleMessage(delay, targetUsers, message, specificUserId, createdBy, botId) {
    const now = Date.now();
...
function executeTask(taskId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            switch (task.targetUsers) {
                case 'specific':
                    if (task.specificUserId) {
                        ...
                        yield notifyAdmin(task.createdBy, `✅ Scheduled message delivered to user ${task.specificUserId}`, task.botId);
                    }
                    break;
                case 'all':
            ...
            try {
                yield notifyAdmin(task.createdBy, `❌ Error sending scheduled message: ${error instanceof Error ? error.message : String(error)}`, task.botId);
            }
            catch (notifyError) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const userId of userIds) {
            try {
                ...
                console.error(`Failed to send scheduled message to ${userId}:`, error);
                failureCount++;
            }
            // Add a small delay between messages to avoid rate limiting
            yield new Promise(resolve => setTimeout(resolve, 100));
        }
        // Report statistics to admin
        yield notifyAdmin(task.createdBy, `✅ Scheduled message batch complete\n\nSuccessful: ${successCount}\nFailed: ${failureCount}\nTotal recipients: ${userIds.length}`, task.botId);
    });
}
/**
 ...
 */
function handleScheduleCommand(msg, botId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (commandArgs.length < 2) {
            yield (0, error_boundary_1.safeSendMessage)(chatId, "⚠️ *Incorrect Format*\n\n" +
                ...
                "- `/schedule 10s 123456789 This is a private message`", { parse_mode: 'Markdown' }, botId);
            return;
        }
        // Parse time
